...
$end = $s.IndexOf('<section class="s b"><div class="ey">08 · 마무리 · Q&amp;A</div>')
if($start -ge 0 -and $end -gt $start){ $s = $s.Remove($start, $end-$start) }
...
$s = $s.Remove($capStart, $capEnd-$capStart).Insert($capStart, $cap)
}
